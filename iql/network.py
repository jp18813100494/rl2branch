...
class GNNPolicy(BaseModel):
    def __init__(self, emb_size=64,cons_nfeats=5,edge_nfeats=1,var_nfeats=19,seed=1):
        super().__init__()
...
class Critic(GNNPolicy):
    ...
    def forward(self, state):
        constraint_features, edge_indices, edge_features, variable_features,candidates,nb_candidates = state
        critic_val = super().forward(constraint_features, edge_indices, edge_features, variable_features)
        critic_val = pad_tensor(critic_val[candidates], nb_candidates)
        return critic_val

class Value(GNNPolicy):
    """
    Critic function for discrete actions
    """
    def __init__(self, hidding_size,seed=1):
