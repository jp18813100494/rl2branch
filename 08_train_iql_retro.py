...
def train(config, args):
    ...
    for epoch in range(0, config['max_epochs']+1):
        ...
        if is_validation_epoch(epoch):
            ...
            logger.info(f"Episode: {epoch} | Batches: {batches} | Valid_nnodes_g: { gmean(np.asarray(v_nnodess) + 1) - 1}  | Valid_time: {np.mean(v_times)} | Valid_lpiters: {np.mean(v_lpiterss)} ")
